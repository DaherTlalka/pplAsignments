...
const evalTraceExp = (exp: TraceExp,env:Env): Result<void> =>
    // complete this print the theng in back
    bind(applyEnvBdg(env,exp.var.var), (binding: FBinding) => isClosure(unbox(binding.val))?
        makeOk(setBox(binding.val,makeTracedClosure(unbox(binding.val) as Closure,exp.var.var))):
        makeFailure("There is no TraceExp"))
...
const applyTracedClosure = (proc: TracedClosure, args: Value[]): Result<Value> =>{ 
    // complete this
    printPreTrace(proc.name,args,counter)
    counter++;
    let val = applyClosure(proc.clo, args)
    counter--;
    printPostTrace(isOk(val)? val.value :" ",counter)
    return val
}
