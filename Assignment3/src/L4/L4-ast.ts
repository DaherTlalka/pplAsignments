...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | SetExp | TraceExp; // HW3
...
// HW3 
export type SpecialFormKeyword = "lambda" | "let" | "letrec" | "if" | "set!" | "quote" | "trace";
const isSpecialFormKeyword = (x: string): x is SpecialFormKeyword =>
    "if"===x || "lambda"===x ||"let"===x|| "quote"===x ||"letrec"===x || "set!"===x || "trace"===x;
/*
...
const isPrimOpKeyword = (x: string): x is PrimOpKeyword =>
    "+"===x || "-"===x || "="===x ||"*"===x || "/"===x|| ">"===x || "<"===x || "not"===x|| "and"===x || "or"===x
    ||"eq?"===x || "string=?"===x || "boolean=?"===x || "cons"===x ||"car"===x || "cdr"===x || "list"===x || "pair?"===x 
    ||"list?"===x ||"number?"===x || "symbol?"===x ||"boolean?"===x|| "string?"===x 
...
// HW3
export const isTraceExp = (x: any): x is TraceExp => x.tag === "TraceExp"// complete this
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    ...
    isLetrecExp(x) || isSetExp(x) || isTraceExp(x); // HW3
...
export const parseL4SpecialForm = (op: SpecialFormKeyword, params: Sexp[]): Result<CExp> =>
    ...
    op === "trace" ? parseTraceExp(params) : // HW3
    op;
...
export const parseTraceExp: (params: Sexp[]) => Result<TraceExp> = 
    (params) =>params.length>1||!isString(params[0])?makeFailure("have to mach exp"):
...
export const unparse = (exp: Parsed): string =>
    ...
    isTraceExp(exp) ? `(trace ${exp.var.var})` :
    exp;
